...
| 1.3 | Tooltips on touch: pointer events; first tap opens the tip instead of the link, second tap follows it. On a doubled lane the tip lists **both** books as links and the finger picks — better than cycling, since a 4px lane can't be aimed at. A peek stops propagation so the anchor-click listener doesn't arm a settle that would re-align to the old hash | 5 | You can discover Philemon behind Colossians on a phone | ✅ synthetic touch events; thumb still wanted |
| 1.4 | Tooltips on focus: `focusin` / `focusout` show the tip centred under the mark | 6 | Tab to a bar, the tooltip appears | ✅ |
| 1.5 | Roving tabindex on the spine **and each chip row**: one tab stop enters, arrow keys move between marks, Home/End jump, Escape leaves. Plus a "Skip to the books" link as the first thing a keyboard reaches | 7 | Tab from the masthead reaches the first paragraph in under ten presses | ✅ 17 stops before Genesis's prose (was ~150); one press with the skip link |
...
| 2.5 | Empires map (Assyria → Babylon → Persia) for Isaiah, Jeremiah, Ezekiel, Daniel, Esther, Nehemiah; Galilee and Judea for the four gospels | 13 | Under ten books left unmapped; decide whether those need one at all | ✅ empires plate ×6, gospels on the land plate ×4; 51 of 66 mapped after Judges, Ruth, both Samuels, Haggai, Zechariah and Malachi took the land plate too. The 15 left are Leviticus, wisdom, Psalms, Lamentations and the short letters — no map wanted |
...
| 3.4 | Live masthead figures: *Time to read* and *Distance* count down as books are ticked off. Needs per-book word counts in `books.py` first | 19 | The masthead answers "how much is left", not "how much is there"  ✅ `WORDS` in `books.py` from the ESV table that sums to the masthead's own 756,846; four cells carry a "Your progress" line |
